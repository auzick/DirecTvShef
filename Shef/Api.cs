...
namespace Shef
{
    public class Api
    {
        ...
        public DateTime CommandGetCurrentTime()
        {
            var result = ProcessCommand("FA91");
            var data = result.returnValue.data;
            var year = int.Parse(data.Substring(0, 2), System.Globalization.NumberStyles.HexNumber) + 1993;
            var month = int.Parse(data.Substring(2, 2), System.Globalization.NumberStyles.HexNumber);
            var day = int.Parse(data.Substring(4, 2), System.Globalization.NumberStyles.HexNumber);
            var hour = int.Parse(data.Substring(6, 2), System.Globalization.NumberStyles.HexNumber);
            var minute = int.Parse(data.Substring(8, 2), System.Globalization.NumberStyles.HexNumber);
            var second = int.Parse(data.Substring(10, 2), System.Globalization.NumberStyles.HexNumber);
        ...
        public Model.Responses.Version GetVersion(string callback = null, string wrapper = null)
        {
            var req = new RestRequest("info/getVersion");
            if (!string.IsNullOrWhiteSpace(callback)) req.AddParameter("callback", callback);
            if (!string.IsNullOrWhiteSpace(wrapper)) req.AddParameter("wrapper", wrapper);
            return Server.MakeRequest<Model.Responses.Version>(req);
        }
